        ...
        switch (key) {
            ...
            case 2://撤销申请
                this.alertInstance = alert('', '确定撤销申请?', [
                    ...
                    {
                        text: '确定', onPress: () => {
                            afterSalesAction.cancelApplyPost(userId, afterSale.refundInfo.aftersaleId, afterSale.refundInfo, this.orderId);
                        }, style: {color: '#000000'}
                    },
                ]);
                break;
    ...
    render() {
        ...
        if (refundInfo !== null) {
            return (<div style={{background: '#F2F2F2'}}>
                ...
                {/**待审核状态可以撤销申请**/}
                {(refundInfo.afterSaleStatus === 1 && refundInfo.isCancelAS == 1 && this.state.isConsumer) && <div className="quitUnClick status-1" 
                onClick={() => {
                        this.onClick(2)
                    }}  style={{color:"red"}}>
                   撤销申请
