...
import { postTwo } from '../../../common/Apis/Fetch';
...
export default class HomePage extends Component {
  ...
  //reader前
  componentWillMount() {
    db.getShopBasic(this.shopId, 2); // 获取店铺基本信息接口
    weidudb.userAuthorization();//调用原生方法获取用户信息
    if (db.readUserInfo() !== null) {
      let data = {
  ...
  componentDidMount() {
    ...
    const { RetailPage, RetailPageActions } = this.props;
    RetailPageActions.isShopFrozen(`/shop/isShopFrozen?shopId=${this.shopId}`);
    RetailPageActions.getShopSubInfoPost(this.shopId, (data) => {
    }, (err) => {
    });
    RetailPageActions.groupingNameList(this.shopId, (data) => {
    }, (err) => {
    });
  ...
  getHomepageListPost() {
    ...
    RetailPageActions.getHomepageList(RetailPage.homePageList, bodyDate, (data, goods) => {
      if (data === 0) {
  ...
  getCategoryView() {//分组
    ...
    return (
      <GroupingList {...this.props}
        ...
        groupingListData={RetailPage.groupingNameData}
  ...
  isShowBlackView() {//是否显示空白界面
    const { RetailPage } = this.props;
    console.log("是否显示空白界面", RetailPage);
    let dic = RetailPage.shopData;
    if(Number(dic.fitmentId) === 5){
      return 5;
    }else if (RetailPage.homePageList.length > 0 || (dic.logoUrl && dic.logoUrl.length > 0) || (dic.describes && dic.describes.length > 0)) {
      return false;
    }else {
      return true;
    }
  ...
  getFooterView() {
    const { RetailPage } = this.props;
    if (RetailPage && RetailPage.homePageList.length > 0) {
      return (
  ...
  contact() { // 在线客服
    ...
    if (userInfo === null) {
      history.push({
        ...
        state: {
          pathname: "//192.168.9.41/customer-service/#/?" + query,
          type: 5
        }
      });
    } else {
  ...
  render() {
    const { RetailPage } = this.props;
    let row = this.getHat();
    let homePageListNumber = RetailPage.homePageList.length;
    ...
    const isShowBlackView = this.isShowBlackView();
    console.log(isShowBlackView,'isShowBlackView');
    return ( //
      <div className="mainView">
        <div className={isShowBlackView === true ? "homePageBodyBlank" : "homePageBody"}>
          <Navigation {...this.props} title={RetailPage.shopData.shopName} shopId={this.shopId} shopName={RetailPage.shopData.shopName} />
          {RetailPage.isShopFrozen ? <Frozen message="该店铺正在闭关修炼中..." /> : isShowBlackView === 5 ?
                <div className="loadingView">
                  <div className="loadingImg">
                  </div>
                </div> : this.isShowBlackView() ?
              <div className="blankView">
            <img src={require('../../images/homePage/blank.png')} />
            <div>店铺正在装修中,敬请期待</div>
          </div> :
            <ListView
              ref="list"
              dataSource={this.state.dataSource.cloneWithRows(RetailPage.homePageList)}
              renderHeader={() => {
                return (<div>
                  {this.getBannerView()}
