...
class ApplyAfterSalesService extends React.Component {
    ...
    handleSubmit() {
        ...
        if (this.state.fileList.length > 0) {
            /* canSubmit = false;
            //上传图片到微信服务器
            upLoadImgToWx(imgAry[0], (ary) => { //获取在微信的临时url
                //1 . 根据微信url 获取file id
                let dic = {
                    con: {
                ...
                console.log(dic)
                postTow('/file/fileUploadIpfs', dic, (data) => {
                    ...
                    if (data.status === 0) {
                        ...
                        {
                            fileIdAry.push(data.body.data[i].fileId)
                        }
                        let params = {
                        ...
                        //Toast.info(data.message);
                        this.setState({showLoading: false});
                        console.log("上传图片 baocuo ")
                        Toast.info(JSON.stringify(data));
                    }
                }, (err) => {
                    console.log("上传图片error ")
                    console.log(err)
                    canSubmit = true;
                    this.setState({showLoading: false});
                    Toast.info(JSON.stringify(err));
            ...
            // console.log("-----------upData------------------------->",upData.userId)
            this.setState({showLoading: true});
                ...
                if (true) {
                    ...
                    if(this.theRequest.type == 4 || Number(this.theRequest.type) > 18) { //已完成订单走新增的申请售后接口
                        let params = {
                            ...
                            saleType: saleType,
                            amount: this.state.inputVa, //退款金额
                            dictionaryId: this.state.reasonValue,
                            describes: this.state.desc,
                            fileId: fileIdAry,
                            orderId: this.theRequest.orderId,
        ...
        return (
            <div className="apply_after">
                ...
                <div className="order-bottom">
                    ...
                    <div className="submit" style={{color: contants.viewStyDic.COLOR1}} onClick={this.handleSubmit.bind(this)}>提交</div>
                    {
               ...
               {/* <div className={"sub"+' '+borderR}  style={{background: contants.viewStyDic.COLOR1, color: contants.viewStyDic.COLOR3}} onClick={this.handleSubmit.bind(this)}>
                    提交
                </div>*/}
                ...
                {/* {
                    this.state.isOpen ? <div>
                        ...
                        <PictureBrowse {...this.props} pictureList={this.state.imgAry} selectIndex={this.state.selectIndex} pictureS={require('../../images/goodDetails/collectIcon.png')}
