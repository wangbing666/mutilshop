...
export default class uploadProof extends Component {
  ...
  submit() {
   ...
   // console.log("this.props.location.state.orderId",this.props.location.state.orderId)
    ...
    if(this.state.fileList.length) {
      ...
      fileUpload(data, (res) => {
        if(res && res[0]){
          ...
          let params = {
            ...
            targetId: this.props.location.state.orderId, //TODO被投诉订单id
            userId: data.userId,
            // picList: fileIds,
            complainDescribe: value,
            orderNo: this.state.orderNumber,
            type: 7 //TODO 4:商户 7:订单 11:商品
          }
          post(`/complain/saveComplainInfo?picList=${fileIds}`, params, res => {
            if(res.status === 0){
      ...
      let params = {
        hintReasonId: reasons,
        targetId: this.props.location.state.orderId, //TODO
      ...
      post(`/complain/saveComplainInfo`, params, res => {
        if(res.status === 0){
          alert("提交成功", "微度平台已经成功收到了您的投诉，我们会尽快处理。如有疑问请联系客服021-6079-0010", [
            {text: '确定', onPress: () => this.goBack()}
          ]);
    ...
    return (
      <div className="name-space-proof">
        ...
        <div className="submit" >
          <button disabled={this.state.submitBtn} style={{color: contants.viewStyDic.COLOR1, background:'#fff'}} onClick={() => this.submit()}>提交反馈</button>
        </div>
